...
interface SplinePointsInputProps {
  points: SplinePoint[];
  ...
  splineDegree: number;
  setSplineDegree: (degree: number) => void;
}
...
export function SplinePointsInput({
  points,
  setPoints,
  ...
  splineDegree,
  setSplineDegree,
}: SplinePointsInputProps) {
  const [isValid, setIsValid] = useState(false);

  useEffect(() => {
    setIsValid(points.length >= splineDegree + 1 && points.some(p => !isNaN(parseValue(p.x)) && !isNaN(parseValue(p.y))));
  }, [points, splineDegree]);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex justify-between items-center">
          <span>Puntos de Interpolación (Spline)</span>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={splineDegree.toString()} onValueChange={(value) => setSplineDegree(parseInt(value))}>
              <SelectTrigger id="spline-degree">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="1">Lineal (grado 1)</SelectItem>
                <SelectItem value="2">Cuadrático (grado 2)</SelectItem>
                <SelectItem value="3">Cúbico (grado 3)</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-gray-500">
          ...
          <div className="grid gap-2 items-center" style={{ gridTemplateColumns: `repeat(${points.length}, minmax(0, 1fr))` }}>
            {points.map((_, index) => (
              <Button
                ...
                disabled={points.length <= splineDegree + 1}
