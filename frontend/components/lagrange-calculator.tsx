...
import { LagrangePointsInput } from '@/components/lagrange-points-input';
import { LagrangeErrorChart } from '@/components/lagrange-error-chart';
...
export function LagrangeCalculator() {
  ...
  const [results, setResults] = useState<LagrangeResponse | null>(null);
  ...
  const handleCalculate = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/lagrange`, {
        method: 'POST',
      ...
      const data: LagrangeResponse = await response.json();
      setResults(data);
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6 p-4">
      <div className="grid grid-cols-1 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Interpolación Polinomial de Lagrange</CardTitle>
            <p className="text-sm text-gray-600 mt-2">
              Ingresa puntos (mínimo 3, máximo 20) para construir un polinomio interpolante usando el método de Lagrange
            </p>
          </CardHeader>
        </Card>
